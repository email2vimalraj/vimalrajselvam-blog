...
summary: 'React Hooks: Lift up / pass down state using useContext and useReducer'
date: '2019-03-13'
tags: ['react']
---
...
function UserList() {
  ...
  return (
    <ul>
      <li>
        <span>Vimalraj Selvam</span>
        <button type="button" onClick={() => dispatch({ type: UPDATE_USER, username: 'Vimalraj' })}>
          Edit
        </button>
      </li>
...
function AddGenderToUser() {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => dispatch({ type: SET_GENDER, gender: '??' })}>
        Add Age
      </button>
    </div>
  )
